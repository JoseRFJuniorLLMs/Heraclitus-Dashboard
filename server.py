...
* /aeb-api/*     -> optional local AEB-STREAM use-case runtime
* /cgee-api/*    -> optional local CGEE budget-integrity runtime
...
ALLOWED_HOSTS = {
    h.strip()
    for h in os.getenv("HERACLITUS_DASHBOARD_ALLOWED_HOSTS", _default_hosts).split(",")
    if h.strip()
}
...
PORTAL_DATASETS = {
    ...
    "viagens": "viagens",
    "despesas-documentos": "despesas/documentos",
    "notas-fiscais": "notas-fiscais",
    "pessoa-juridica": "pessoa-juridica",
    "acordos-leniencia": "acordos-leniencia",
    "ceaf": "ceaf",
    "cartoes": "cartoes",
}
PNCP_RESOURCES = {
...
def _cgee_target(route: str, query: str) -> str | None:
    """Normalize the tiny CGEE read surface and cap expensive timeline scans."""
    ...
    if route == "/api/why":
        if set(params) != {"portaria"} or len(params.get("portaria", [])) != 1:
            return None
        portaria = params["portaria"][0].strip()
        if not portaria or len(portaria) > 160:
            return None
...
class Handler(BaseHTTPRequestHandler):
    server_version = "HeraclitusDashboard/8"
    ...
    def _serve_static(self, path: str):
        ...
        if file_path is None or (file_path != ROOT / "index.html" and ROOT not in file_path.parents) or not file_path.is_file():
    ...
    def _auth_headers(self, require_auth=False, *, core_fallback=False):
        # Browser credentials take precedence. The server-side Basic credential is
        # a Core-only fallback and is NEVER reused for Agent, LABRA, AEB, CGEE or
        # public-data upstreams.
        auth = self.headers.get("Authorization", "")
        if not auth and core_fallback and CORE_AUTH_HEADER:
            auth = CORE_AUTH_HEADER
        if require_auth and (not auth.startswith(("Basic ", "Bearer ")) or len(auth) > 8192):
            return None
        headers = {"Accept": "application/json", "User-Agent": "Heraclitus-Dashboard/8"}
        if auth and len(auth) <= 8192:
    ...
    def _proxy(
        ...
        *,
        ...
        extra_headers=None,
        timeout=15,
        forward_browser_auth=True,
    ):
        headers = self._auth_headers(require_auth, core_fallback=core_fallback) if forward_browser_auth else {
            "Accept": "application/json",
            "User-Agent": "Heraclitus-Dashboard/8",
        }
        if headers is None:
    ...
    def _proxy_sse(self, host: str, port: int, target: str, *, require_auth=True, core_fallback=False, timeout=75):
        ...
        try:
            ...
            if response.status != 200:
                body = response.read(MAX_RESPONSE + 1)
                return self.send_body(response.status, body[:MAX_RESPONSE], response.getheader("Content-Type", "application/json; charset=utf-8"))
            self.send_response(200)
            self.send_header("Content-Type", response.getheader("Content-Type", "text/event-stream; charset=utf-8"))
            ...
            if not self.wfile.closed:
                try:
                    self.wfile.write(b"event: dashboard-error\ndata: {\"error\":\"UPSTREAM_UNAVAILABLE\"}\n\n")
                    self.wfile.flush()
                except OSError:
                    pass
        finally:
    ...
    def _public_status(self):
        return self.send_body(200, _json_bytes({
            "portal_transparencia": {
                "configured": bool(PORTAL_API_KEY),
                "credential_configured": bool(PORTAL_API_KEY),
                "proxy_enabled": True,
                "upstream_checked": False,
                "base": "https://api.portaldatransparencia.gov.br/api-de-dados",
                "datasets": sorted(PORTAL_DATASETS),
                "credential": "server-side env PORTAL_TRANSPARENCIA_API_KEY",
            },
            "pncp": {
                "configured": True,
                "proxy_enabled": True,
                ...
                "resources": sorted(PNCP_RESOURCES),
                "credential": "not required for these public reads",
            },
    ...
    def _public_portal(self, dataset, query):
        ...
        if not endpoint:
            return self.error(403, "Dataset do Portal fora da allowlist", code="PUBLIC_DATASET_DENIED")
        if not PORTAL_API_KEY:
            return self.error(503, "Configure PORTAL_TRANSPARENCIA_API_KEY no processo do dashboard", code="PORTAL_API_KEY_MISSING")
        ...
        return self._proxy(
            ...
            target,
            https=True,
            extra_headers={"chave-api-dados": PORTAL_API_KEY},
            timeout=30,
            forward_browser_auth=False,
        )

    def _public_pncp(self, resource, query):
        endpoint = PNCP_RESOURCES.get(resource)
        if not endpoint:
            return self.error(403, "Recurso PNCP fora da allowlist", code="PUBLIC_RESOURCE_DENIED")
        return self._proxy(
            "pncp.gov.br",
            443,
            endpoint + (f"?{query}" if query else ""),
            https=True,
            timeout=30,
            forward_browser_auth=False,
        )
    ...
    def do_GET(self):
        if not self._host_ok():
            return self.error(403, "Host não autorizado", code="HOST_DENIED")
        if len(self.path) > MAX_PATH:
            return self.error(414, "Consulta demasiado longa", code="URI_TOO_LONG")
        parsed = urlsplit(self.path)
        if not _valid_query(parsed.query):
            return self.error(400, "Query contém caracteres não permitidos", code="BAD_QUERY")

        if parsed.path == "/dashboard-api/status":
            return self.send_body(200, _json_bytes({
                "product": "HeraclitusDB Platform Console",
                "release": RELEASE,
                "read_only": True,
                "core_auth_mode": "server_env" if CORE_AUTH_HEADER else "browser_memory",
                "labra_runtime": {"proxy_enabled": True, "port": LABRA_PORT},
                "aeb_runtime": {"proxy_enabled": True, "port": AEB_PORT},
                "cgee_runtime": {"proxy_enabled": True, "port": CGEE_PORT},
            }))

        if parsed.path == "/public-api/status":
            return self._public_status()
        if parsed.path.startswith("/public-api/portal/"):
            return self._public_portal(parsed.path.removeprefix("/public-api/portal/"), parsed.query)
        if parsed.path.startswith("/public-api/pncp/"):
            return self._public_pncp(parsed.path.removeprefix("/public-api/pncp/"), parsed.query)

        if parsed.path.startswith("/labra-api/"):
            route = parsed.path.removeprefix("/labra-api")
            if not LABRA_READ_ROUTES.fullmatch(route):
                return self.error(403, "Rota LABRA fora do escopo somente-leitura", code="ROUTE_DENIED")
        ...
        if parsed.path.startswith("/aeb-api/"):
            ...
            if not AEB_READ_ROUTES.fullmatch(route):
                return self.error(403, "Rota AEB fora do escopo somente-leitura", code="ROUTE_DENIED")
        ...
        if parsed.path.startswith("/cgee-api/"):
            ...
            if not CGEE_READ_ROUTES.fullmatch(route):
                return self.error(403, "Rota CGEE fora do escopo somente-leitura", code="ROUTE_DENIED")
            target = _cgee_target(route, parsed.query)
            if target is None:
                return self.error(400, "Parâmetros CGEE inválidos ou fora dos limites", code="BAD_QUERY")
            timeout = 45 if route == "/api/verify" else 25
            return self._proxy(CGEE_HOST, CGEE_PORT, target, timeout=timeout, forward_browser_auth=False)

        if parsed.path.startswith("/agent-api/"):
            route = parsed.path.removeprefix("/agent-api")
            if not AGENT_READ_ROUTES.fullmatch(route):
                return self.error(403, "Rota Agent fora do escopo somente-leitura", code="ROUTE_DENIED")
        ...
        if parsed.path.startswith("/api/"):
            ...
            if not CORE_READ_ROUTES.fullmatch(route):
                return self.error(403, "Rota Core fora do escopo somente-leitura", code="ROUTE_DENIED")
            target = route + (f"?{parsed.query}" if parsed.query else "")
            if route == "/live/events":
                return self._proxy_sse(CORE_HOST, CORE_PORT, target, core_fallback=True)
            return self._proxy(
                CORE_HOST,
                CORE_PORT,
                target,
                require_auth=(route != "/healthz"),
                core_fallback=True,
                timeout=65 if route.startswith("/verify") else 20,
            )
...
if __name__ == "__main__":
    ...
    print(
        f"HeraclitusDB Platform Dashboard {RELEASE} em http://{DASHBOARD_BIND}:{DASHBOARD_PORT} "
        f"· core-auth={auth_mode} · labra={LABRA_HOST}:{LABRA_PORT} · aeb={AEB_HOST}:{AEB_PORT} "
        f"· cgee={CGEE_HOST}:{CGEE_PORT}"
    )
