...
assert.deepEqual(cssLinks, ['css/styles.css', 'css/platform.css', 'css/labra-case.css', 'css/aeb-case.css'], 'runtime must load base shell and isolated use-case stylesheets only');
...
for (const required of [
  ...
  'LABRA-AGU','AEB-STREAM','Agent Black Box','Sentinel / SOC','Capacidades & runtime',"const RELEASE = '2026.09.14-r7'"
...
for (const required of ['.nav-rail','.nav-context','.nav-context-links','.command-palette','html.nav-panel-collapsed .wrap','html.nav-mobile-open #nav','.nav-backdrop','@media (max-width: 900px)','#soc .soc']) assert(platform.includes(required), `platform shell CSS is missing ${required}`);

assert(labraCss.includes('.labra-case .labra-hero'));
assert(labraCss.includes('.labra-case .labra-tabs'));
assert(!/(^|})\s*(?:body|header|nav|main|aside|html|\*)\b/.test(labraCss), 'LABRA CSS leaks a global selector');
assert(aebCss.includes('.aeb-case .aeb-hero'));
assert(aebCss.includes('.aeb-case .aeb-tabs'));
assert(aebCss.includes('.aeb-case .aeb-architecture'));
assert(!/(^|})\s*(?:body|header|nav|main|aside|html|\*)\b/.test(aebCss), 'AEB CSS leaks a global selector');
...
console.log('UI shell contracts OK: R7 rail/context navigation, isolated LABRA/AEB surfaces, command palette, breadcrumb and mobile drawer.');