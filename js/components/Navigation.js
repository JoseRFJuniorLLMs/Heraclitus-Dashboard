...
const AREAS = [
  ...
  {id:'cases',label:'Casos',icon:ICONS.cases,defaultRoute:'cases',description:'Aplicações completas construídas sobre HeraclitusDB e casos persistidos no Core.',routes:[['cases','Catálogo de casos'],['labra','LABRA-AGU'],['aeb','AEB-STREAM'],['cgee','CGEE · Integridade Orçamentária'],['frd','SPEC-FRD · Integridade Pública']]},
...
export const Navigation = {
  ...
  render(){
    ...
    return `<div class="app-nav"><div class="nav-rail" aria-label="Áreas da plataforma">
      ...
      <div class="rail-stack rail-primary">${primary.map(railButton).join('')}</div><div class="rail-spacer"></div><div class="rail-stack rail-utility">${utility.map(railButton).join('')}</div>
  ...
  init(){
    ...
    document.getElementById('nav-command-button')?.addEventListener('click',openCommand);document.addEventListener('hera:open-command',openCommand);
    palette?.addEventListener('click',event=>{if(event.target.closest('[data-command-close]'))return closeCommand();const result=event.target.closest('[data-route]');if(!result)return;closeCommand();go(result.dataset.route,{history:true});});
    commandInput?.addEventListener('input',()=>{commandIndex=0;renderCommandResults(commandInput.value);});commandInput?.addEventListener('keydown',event=>{if(event.key==='ArrowDown'){event.preventDefault();commandIndex=Math.min(commandIndex+1,commandMatches.length-1);renderCommandResults(commandInput.value);}else if(event.key==='ArrowUp'){event.preventDefault();commandIndex=Math.max(0,commandIndex-1);renderCommandResults(commandInput.value);}else if(event.key==='Enter'&&commandMatches[commandIndex]){event.preventDefault();const route=commandMatches[commandIndex].id;closeCommand();go(route,{history:true});}else if(event.key==='Escape'){event.preventDefault();closeCommand();}});
    document.addEventListener('keydown',event=>{if((event.ctrlKey||event.metaKey)&&event.key.toLowerCase()==='k'){event.preventDefault();openCommand();}else if(event.key==='Escape'){closeCommand();closeMobile();}});
    document.addEventListener('hera:navigate',event=>go(event.detail,{history:true}));window.addEventListener('popstate',()=>go(location.hash.slice(1)||'overview',{history:false,focus:false}));const initial=ROUTE_INDEX.has(location.hash.slice(1))?location.hash.slice(1):'overview';const entry=ROUTE_INDEX.get(initial);renderArea(entry.area);go(initial,{history:true,replace:true,focus:false});
  },
};
