...
export const Navigation = {
    ...
    return `<div class="app-nav">
      ...
      <div class="nav-context" id="nav-context">
        ...
        <div class="nav-context-footer">
          <button id="nav-command-button" class="nav-command-button" type="button"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6"/><path d="m16 16 4 4"/></svg><span>Ir para qualquer tela</span><kbd>Ctrl K</kbd></button>
  ...
  init(){
    ...
    const markRoute=id=>document.querySelectorAll('#nav-context-links a[data-s]').forEach(link=>{
      const active=link.dataset.s===id; link.classList.toggle('active',active);
    ...
    const go=(id,{history=false,replace=false,focus=true}={})=>{
      const entry=ROUTE_INDEX.get(id), target=document.getElementById(id); if(!entry||!target)return;
      if(entry.area.id!==this.activeArea) renderArea(entry.area);
      this.activeRoute=id; markRoute(id);
      document.querySelectorAll('#main-content > section').forEach(section=>section.classList.toggle('on',section.id===id));
      if(history&&location.hash!==`#${id}`){ if(replace) window.history.replaceState({route:id},'',`#${id}`); else window.history.pushState({route:id},'',`#${id}`); }
    ...
    mobileToggle?.addEventListener('click',toggleMobile);backdrop?.addEventListener('click',closeMobile);document.addEventListener('hera:toggle-nav',toggleMobile);
    ...
    const renderCommandResults=query=>{
      const q=fold(query);commandMatches=allRoutes().filter(item=>!q||fold(`${item.label} ${item.area.label} ${item.area.description}`).includes(q)).slice(0,12);commandIndex=Math.min(commandIndex,Math.max(0,commandMatches.length-1));if(!commandResults)return;
    ...
    const initial=ROUTE_INDEX.has(location.hash.slice(1))?location.hash.slice(1):'overview';const entry=ROUTE_INDEX.get(initial);renderArea(entry.area);go(initial,{history:true,replace:true,focus:false});
  },
};
