...
export const LoginModal={
  ...
  async bootstrap(){
    ...
    const savedCred = sessionStorage.getItem('hera_core_cred');
    const savedAgent = sessionStorage.getItem('hera_agent_token');
    if(savedCred) API.definirCredenciais(savedCred);
    if(savedAgent) API.definirAgentToken(savedAgent);
    const check=await API.stats();
    if(check.ok){
      this.autoShown=false;
      const user = savedCred ? savedCred.split(':')[0] : (this.serverAuth ? 'admin (.env)' : null);
      this.badge(user);
      document.dispatchEvent(new CustomEvent('hera:core-authenticated',{detail:{mode:savedCred?'browser_session':(this.serverAuth?'server_env':'browser_memory')}}));
      return true;
    }
    if(check.falha==='auth' || !savedCred){this.show('auth');return false;}
  ...
  init(){
    ...
    document.getElementById('btnCloseLoginModal').onclick=()=>this.hide();
    document.addEventListener('hera:open-login',()=>this.show('manual'));
    document.addEventListener('hera:auth-required',()=>{if(!this.autoShown)this.show('auth');});
    document.getElementById('btnLogout').onclick=async()=>{
      API.definirCredenciais(null);API.definirAgentToken(null);
      sessionStorage.removeItem('hera_core_cred');
      sessionStorage.removeItem('hera_agent_token');
      this.autoShown=false;
    ...
    f.onsubmit=async e=>{
      e.preventDefault();err.hidden=true;submit.disabled=true;submit.textContent='A verificar…';
      const core=API.definirBase(document.getElementById('loginEndpoint').value);
      if(core.erro){err.textContent=core.erro;err.hidden=false;submit.disabled=false;submit.textContent='Conectar ao HeraclitusDB';return;}
      const ag=API.definirAgentBase(document.getElementById('loginAgentEndpoint').value);
      if(ag.erro){err.textContent=ag.erro;err.hidden=false;submit.disabled=false;submit.textContent='Conectar ao HeraclitusDB';return;}
      const user=document.getElementById('loginUser').value.trim(),pass=document.getElementById('loginPass').value;
      const agToken = document.getElementById('loginAgentToken').value.trim();
      API.definirCredenciais(`${user}:${pass}`);
      API.definirAgentToken(agToken);
      const r=await API.stats();
      submit.disabled=false;submit.textContent='Conectar ao HeraclitusDB';
      if(!r.ok){
        API.definirCredenciais(null);
        sessionStorage.removeItem('hera_core_cred');
        const x=explicarFalha(r.falha,r.estado);
        err.textContent=x.longo;err.hidden=false;this.badge();return;
      }
      sessionStorage.setItem('hera_core_cred', `${user}:${pass}`);
      if(agToken) sessionStorage.setItem('hera_agent_token', agToken);
      this.serverAuth=false;this.autoShown=false;
      const ar=await API.agentGet('/api/v1/agent/status',{ms:5000});
      this.badge(user);
      document.dispatchEvent(new CustomEvent('hera:endpoint-mudou'));
      document.dispatchEvent(new CustomEvent('hera:core-authenticated',{detail:{mode:'browser_session'}}));
      if(!ar.ok&&ar.falha==='auth'){
        err.textContent='Core conectado. O Agent Black Box exige um Bearer/OIDC separado para consultar runs e evidências.';
  ...
  badge(user){
    const b=document.getElementById('userProfileBadge');if(!b)return;
    const cred=API.credenciais(),ag=API.agentTokenConfigurado();
    if(cred){
      b.textContent=`Core: ${user||cred.split(':')[0]}${ag?' · Agent':''}`;
      b.classList.add('connected');
      b.title='Clique para trocar usuário ou desconectar';
      return;
    }
    if(this.serverAuth){
      b.textContent='Core: admin (.env)';
      b.classList.add('connected');
      b.title='Conectado via .env local. Clique para trocar credenciais';
      return;
    }
    b.textContent='Autenticar';
    b.classList.remove('connected');
    b.title='Clique para autenticar no HeraclitusDB';
  }
};
