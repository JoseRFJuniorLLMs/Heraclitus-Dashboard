...
def replace_once(path, old, new):
    ...
    if old not in text: raise SystemExit(f'marker missing in {path}: {old!r}')
...
if "RedTeamSecurity.js" not in t:
    anchor="const agent = fs.readFileSync('js/components/AgentBlackBox.js','utf8');\n"
    if anchor not in t: raise SystemExit('contracts agent marker missing')
    t=t.replace(anchor,anchor+"const redteam = fs.readFileSync('js/components/RedTeamSecurity.js','utf8');\n",1)
    tail="assert(agent.includes('/api/v1/agent/runs'));"
    if tail not in t: raise SystemExit('contracts assertion marker missing')
    t=t.replace(tail,tail+"\nassert(redteam.includes('/api/v1/agent/red-team/events'));\nassert(redteam.includes('Contrato de verdade'));\nassert(app.includes('RedTeamSecurity'));\nassert(nav.includes(\"['redteam','Red Team / Agent Security']\"));\nassert(server.includes('red-team/events'));",1)
p.write_text(t)
